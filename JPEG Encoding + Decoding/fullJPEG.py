# jpeg Full pipeline
# Authors: Kritika Pandit, Anika Rajbhandari
...
def upsample_nn(channel_small: np.ndarray, target_shape: Tuple[int, int]):
    """
    ...
    # Repeat each pixel 2x vertically and 2x horizontally
    up = np.repeat(np.repeat(channel_small, 2, axis = 0), 2, axis = 1)
    
    # Trim in case target isn't exactly 2x (e.g., due to padding elsewhere)
    return up[:H, :W]
...
def build_huffman_codes(symbols: List[Any]) -> Dict[str, str]:
    ...
    # 4) Walking the tree and assign codes: left = 0, right = 1
    codes: Dict[str, str] = {}
...
def encode_channel(channel_u8: np.ndarray, base_qt: np.ndarray, quality: int, print_first_block: bool = False) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    bitstring = huffman_encode(all_tokens, huff_codes)
...
def jpeg_decode_pipeline(meta: Dict[str, Any]) -> Image.Image:
    ...
    Cb = upsample_nn(Cb_ds, (H, W))
    Cr = upsample_nn(Cr_ds, (H, W))
...
def main():
    parser = argparse.ArgumentParser(description="JPEG Pipe;ome")
